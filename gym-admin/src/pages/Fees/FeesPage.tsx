...
type AuditUser = { name: string; role: string };
type Member = { id: number; name: string; email: string; phone: string; isActive: boolean; createdAt: string; updatedAt: string; createdBy?: AuditUser | null; updatedBy?: AuditUser | null; memberships?: Membership[] };
...
export function FeesPage() {
  ...
  const canEditMember = hasPermission(Permission.MembersEdit);
  const membersPage = location.pathname === "/members";
  ...
  async function toggleMemberStatus() {
    if (!selectedMember || !canEditMember) return;
    setSaving(true);
    try {
      const updated = await apiFetch<Member>(`/api/members/${selectedMember.id}`, {
        method: "PUT",
        body: JSON.stringify({
      ...
      setMessage({ type: "success", text: `${updated.name} is now ${updated.isActive ? "active" : "inactive"}.` });
    } catch (error) {
      setMessage({ type: "error", text: error instanceof Error ? error.message : "Unable to update member status." });
    } finally {
      setSaving(false);
    }
  }
  ...
  const matchingMembers = members.filter((member) => `${member.name} ${member.email} ${member.phone}`.toLowerCase().includes(memberSearch.toLowerCase()));
  const memberPageCount = Math.max(1, Math.ceil(matchingMembers.length / 100));
  const visibleMembers = matchingMembers.slice((memberPage - 1) * 100, memberPage * 100);
  ...
  return <main className={`page cms-page ${membersPage ? "members-page" : "payments-page"}`}>
    <div className="page-header"><div><h1>{location.pathname === "/members" ? "Members" : "Payments & Fees"}</h1><p className="muted">{location.pathname === "/members" ? "Manage members, memberships and their payment history." : "Record payments and review transaction history."}</p></div></div>
    {message ? <p className={`info ${message.type}`}>{message.text}</p> : null}
    {membersPage && canCreateMember ? <section className="card cms-form"><h2>Add Member</h2><div className="form-grid">
    ...
    {membersPage && canCreateMember ? <section className="card cms-form import-card"><div className="import-heading"><div><p className="eyebrow">BULK ONBOARDING</p><h2>Import members</h2><p className="muted">Upload an Excel or CSV file to add members, memberships and payments.</p></div><label className="btn upload-button"><input type="file" accept=".xlsx,.xls,.csv" onChange={(event) => void readImport(event)} />Choose spreadsheet</label></div><p className="field-hint">Columns: Name, Email, Phone, StartDate, EndDate, Amount, PaymentDate, PaymentMethod.</p>{importRows.length ? <><div className="import-preview"><strong>{importRows.length} rows ready</strong>{importRows.slice(0, 3).map((row, index) => <span key={`${row.email}-${index}`}>{row.name} · {row.email}</span>)}</div><button className="btn" disabled={importing} onClick={() => void importMembers()}>{importing ? "Importing…" : "Import valid rows"}</button></> : null}</section> : null}
    {membersPage ? <section className="member-directory card"><div className="member-directory-header"><div><p className="eyebrow">MEMBERS</p><h2>Member directory</h2><p className="muted">Showing {matchingMembers.length ? ((memberPage - 1) * 100) + 1 : 0}–{Math.min(memberPage * 100, matchingMembers.length)} of {matchingMembers.length} members.</p></div><input className="member-search" placeholder="Search members..." value={memberSearch} onChange={(event) => { setMemberSearch(event.target.value); setMemberPage(1); }} /></div><div className="member-list">{visibleMembers.map((member) => <button type="button" className={`member-row${member.id === form.memberId ? " selected" : ""}`} key={member.id} onClick={() => void selectMember(member.id)}><span className="member-avatar">{member.name.charAt(0).toUpperCase()}</span><span className="member-row-info"><strong>{member.name}</strong><small>{member.email}</small></span><span className={`member-status ${member.isActive ? "active" : "inactive"}`}>{member.isActive ? "Active" : "Inactive"}</span><span className="member-row-arrow">›</span></button>)}</div><div className="member-pagination"><button className="btn secondary" disabled={memberPage <= 1} onClick={() => setMemberPage((page) => page - 1)}>Previous</button><span>Page {memberPage} of {memberPageCount}</span><button className="btn secondary" disabled={memberPage >= memberPageCount} onClick={() => setMemberPage((page) => page + 1)}>Next</button></div></section> : null}
    {membersPage && selectedMember ? <section className="member-profile card"><div className="member-profile-main"><span className="profile-avatar">{selectedMember.name.charAt(0).toUpperCase()}</span><div><p className="eyebrow">MEMBER DETAIL</p><h2>{selectedMember.name}</h2><p className="muted">{selectedMember.email}{selectedMember.phone ? ` • ${selectedMember.phone}` : ""}</p><span className={`member-status ${selectedMember.isActive ? "active" : "inactive"}`}>{selectedMember.isActive ? "Active member" : "Inactive member"}</span>{canEditMember ? <button className={`status-action ${selectedMember.isActive ? "deactivate" : "activate"}`} disabled={saving} onClick={() => void toggleMemberStatus()}>{saving ? "Saving…" : selectedMember.isActive ? "Deactivate member" : "Activate member"}</button> : null}</div></div><div className="member-profile-stats"><div><span>Memberships</span><strong>{selectedMemberships.length}</strong></div><div><span>Current plan</span><strong>{plans.find((plan) => plan.id === selectedMemberships.find((membership) => membership.status.toLowerCase() === "active")?.membershipPlanId)?.name ?? "Not assigned"}</strong></div><div><span>Joined</span><strong>{new Date(selectedMember.createdAt).toLocaleDateString()}</strong></div><div className="member-audit"><span>Added by</span><strong>{selectedMember.createdBy ? `${selectedMember.createdBy.name} · ${selectedMember.createdBy.role}` : "Added before tracking was enabled"}</strong><small>{new Date(selectedMember.createdAt).toLocaleString()}</small><span>Last updated by</span><strong>{selectedMember.updatedBy ? `${selectedMember.updatedBy.name} · ${selectedMember.updatedBy.role}` : "No update recorded"}</strong><small>{new Date(selectedMember.updatedAt).toLocaleString()}</small></div></div>{selectedMemberships.length ? <div className="member-memberships">{selectedMemberships.map((membership) => <div className="member-membership-row" key={membership.id}><strong>{plans.find((plan) => plan.id === membership.membershipPlanId)?.name ?? "Custom membership"}</strong><span>{new Date(membership.startDate).toLocaleDateString()} – {new Date(membership.endDate).toLocaleDateString()}</span><span className={`expiry-status ${membership.status.toLowerCase() === "active" ? "soon" : "expired"}`}>{membership.status}</span></div>)}</div> : <p className="empty-state">No memberships assigned to this member.</p>}</section> : null}
    {!membersPage ? <section className="card cms-form"><h2>Record Payment</h2><div className="form-grid">
    ...
    </div><div className="cms-form-footer cms-form-footer-end"><button className="btn" disabled={!canCreate || saving} onClick={() => void save()}>{saving ? "Saving…" : "Save Payment"}</button></div></section> : null}
    {membersPage && form.memberId && canCreateMember ? <section className="card cms-form"><h2>Add Membership</h2><p className="muted">Assign a plan and dates to the selected member.</p><div className="form-grid">

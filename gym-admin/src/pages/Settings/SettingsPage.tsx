...
export function SettingsPage() {
  ...
  function handleLogoUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 2 * 1024 * 1024) { setMessage({ type: "error", text: "Logo must be smaller than 2 MB." }); return; }
    const reader = new FileReader();
    reader.onload = () => { if (typeof reader.result === "string") update("logoUrl", reader.result); };
    reader.onerror = () => setMessage({ type: "error", text: "Unable to read the logo file." });
  ...
  return <main className="page cms-page"><div className="page-header"><div><h1>Gym Settings</h1><p className="muted">Manage global website configuration.</p></div></div>{message ? <p className={`info ${message.type}`}>{message.text}</p> : null}{loading ? <p>Loading gym settings…</p> : <section className="card cms-form cms-single-form"><div className="form-grid"><div className="form-field"><label>Gym Name<input required value={form.gymName} onChange={(event) => update("gymName", event.target.value)} /></label></div><div className="form-field"><label>Tagline<input value={form.tagline} onChange={(event) => update("tagline", event.target.value)} /></label></div><div className="form-field"><label>Website URL<input type="url" value={form.websiteUrl} onChange={(event) => update("websiteUrl", event.target.value)} /></label></div><div className="form-field"><label>Currency<input required maxLength={3} value={form.currency} onChange={(event) => update("currency", event.target.value.toUpperCase())} /></label></div></div><div className="form-field"><label>Logo URL</label><div className="image-url-row"><input value={form.logoUrl.startsWith("data:") ? "" : form.logoUrl} placeholder="https://example.com/logo.png" onChange={(event) => update("logoUrl", event.target.value)} /><label className="btn upload-button"><input type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml" onChange={handleLogoUpload} />Upload logo</label></div><small className="field-hint">PNG, JPG, WebP or SVG, maximum 2 MB.</small>{form.logoUrl ? <img className="settings-logo-preview" src={form.logoUrl} alt="Gym logo preview" /> : null}</div><div className="cms-form-footer cms-form-footer-end"><button className="btn" onClick={save} disabled={saving || !canEdit}>{saving ? "Saving…" : "Save Changes"}</button></div></section>}</main>;
}
